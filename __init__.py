...
logging.basicConfig(filename = '/var/log/robot/jasper.log', level = logging.DEBUG)
...
class Jasper(object):
    def __init__(self):
        ...
        if not check_network_connection():
            self.mic.speaker.play(jasperpath.data('audio', 'beep_hi.wav'))

    def run(self):
        print("[VOICE SERVICE START]")
        salutation = u"我可以帮你做些什么？"
        self.mic.say(salutation)
        conversation = Conversation(self.getWakeupWord(), self.mic, self.config)
    ...
    def setWakeupWord(self, word):
        f = open('/home/pi/WakeupWord', 'w')
        f.write(word)
        f.close()

    def getWakeupWord(self):
        f = open('/home/pi/WakeupWord', 'r')
        words = f.readlines()
        f.close()
        return words[0]
...
try:
    ...
    print("[ERROR] Error when init Jasper.")
